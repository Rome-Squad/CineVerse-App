...
fun HistoryAndRecentItems(
    ...
    val isJustHistory = resultSearchKeyword.all { it.isFromSearchHistory }
    Column(modifier = modifier) {
        if (resultSearchKeyword.isNotEmpty()) {
            LazyColumn(
                ...
                stickyHeader {
                    SectionTitle(
                        ...
                        title = if (isJustHistory) stringResource(R.string.history) else stringResource(
                            R.string.search_suggestions
                        ),
                        clickableText = if (isJustHistory) stringResource(R.string.clear_all) else "",
                        onClickableText = onClickClearAll
                    )
                }
                items(resultSearchKeyword) { keyWord ->
                    SearchItem(
                        modifier = Modifier
...
private fun HistoryAndRecentItemsPreview() {
    ...
    val today = Clock.System.todayIn(tz).atStartOfDayIn(tz)
    val history = List(2) {
        SearchKeyword(
            keyword = "batman $it",
            isFromSearchHistory = true,
            lastSearchedTime = today.toLocalDateTime(tz)
        )
    }
    val searchResult = List(0) {
        SearchKeyword(
            keyword = "superman $it",
    ...
    CineVerseTheme {
        HistoryAndRecentItems(
            ...
            onClickItem = {},
            onClickIcon = {},
            resultSearchKeyword = history + searchResult,
            recentViews = listOfMovies
        )
    }
}